...
--  GET DEPARTMENT NAME WITH AVERAGE salary** 
SELECT department_name, avg(e.salary) as "Salary" FROM departments as d
...
SELECT extract(year from hire_date) as year, count(*) FROM departments
    JOIN employees USING(department_id)
        GROUP  BY year
...
SELECT customer_id, count(*), sum(total_amount) as "Total Amount" FROM orders
   GROUP BY customer_id
        Having count(*) > 2


SElECT to_char(order_date::date, 'MONTH'), count(*) FROM orders as o
