-- Active: 1720628377068@@127.0.0.1@5432@ph@public
...
--  Filter all employees Whoose salary is greater than HR Department highest salary *** 
...
-- * Sub Queries With SELECT Clause ** 
SELECT * FROM  employees WHERE salary > (
  SELECT max(salary) FROM  employees WHERE department_name = 'HR'
)


--  * SUB Quries With Where clause ** 
SELECT * FROM employees WHERE department_name in (
  SELECT DISTINCT(department_name) FROM employees WHERE department_name LIKE '%E%'
)



SELECT * FROM (
   SELECT department_name, employee_name FROM employees
)
